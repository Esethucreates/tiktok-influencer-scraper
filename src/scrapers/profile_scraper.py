...
@dataclass
class AuthorProfile:
    """Data structure for author profile information"""
    user_id: str
    username: str
    display_name: str
    avatar_url: str
...
class TikTokSearchScraper(CDPXHRMonitor):
    ...
    1. Performing hashtag-based searches on TikTok
    2. Capturing the API responses for search results
    3. Extracting author profiles from search results
    4. Deduplicating and ranking profiles by engagement
    """
    ...
    def __init__(self, hashtags: List[str], max_profiles_per_hashtag: int = 100,
        ...
        """
        Initialize the TikTok Search Scraper
        ...
        """
        # We'll set target_url dynamically, so use a placeholder
        super().__init__(
    ...
    def _build_search_url(self, hashtag: str) -> str:
        ...
        # TikTok search URL structure
        search_url = f"https://www.tiktok.com/search?q={encoded_term}&t={timestamp}"
        return search_url

    def _extract_author_from_item(self, item_data: Dict[str, Any]) -> Optional[AuthorProfile]:
        """
        Extract author profile information from TikTok search result item

        Args:
            item_data: The 'item' field from search results

        Returns:
            AuthorProfile object or None if extraction fails
        """
        try:
            # Extract author data
    ...
    def _process_search_response(self, response_data: Dict[str, Any], hashtag: str) -> List[AuthorProfile]:
        ...
        Args:
            response_data: The captured API response data
            hashtag: The hashtag being searched

        Returns:
            List of extracted AuthorProfile objects
        """
        profiles = []
        ...
        try:
            ...
            for result in data:
                ...
                if profile:
                    ...
                    # Store in main profiles dict (deduplicated by user_id)
    ...
    async def search_hashtag(self, hashtag: str) -> List[AuthorProfile]:
        ...
        Returns:
            List of AuthorProfile objects found
        """
        print(f"\n🔍 Starting search for hashtag: {hashtag}")

        # Build search URL and set it as target_url
        search_url = self._build_search_url(hashtag)
        self.target_url = search_url

        print(f"📍 Will navigate to: {search_url}")

        # Clear previous responses
        self.matched_responses = []

        # Use the parent's run() method to properly monitor network requests
        ...
        # Sort by engagement (follower_count + heart_count) and limit
    ...
    async def search_all_hashtags(self) -> Dict[str, List[AuthorProfile]]:
        """
        Search all configured hashtags and return organized results

        Returns:
            Dictionary mapping hashtags to their found profiles
        """
        results = {}
...
async def search_scraper():
    """Test function to demonstrate the search scraper"""
    hashtags = ["dance", "viral"]

    scraper = TikTokSearchScraper(
        hashtags=hashtags,
        max_profiles_per_hashtag=50,  # Smaller number for testing
        scroll_count=3  # Fewer scrolls for testing
    )
    ...
    print(f"\n📊 SEARCH SUMMARY:")
    print(f"Total unique profiles found: {stats['total_unique_profiles']}")
    print(f"Profiles per hashtag: {stats['hashtag_profile_counts']}")

    # Save results
    scraper.save_results("test_search_results.json")

    return results


if __name__ == "__main__":
    asyncio.run(search_scraper())
